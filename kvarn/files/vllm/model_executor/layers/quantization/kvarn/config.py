...
class KVarNConfig:
    ...
    def tile_bytes_aligned(self) -> int:
        ...
        port(0.27.1): the pow2 slot rounding is now OPT-IN via KVARN_POW2_SLOT=1
        (only needed for heterogeneous-head_dim models such as Gemma-4). The
        default rounds the tile up to lcm(8, group) bytes, so the per-token slot
        ``tile_bytes_aligned // group`` is exact and tight: k4v2_g128 @ D=256 is
        26880 B = 210 B/token/head (pow2 padded it to 32768 = 256 B/token/head,
        i.e. +22% KV-cache footprint for nothing on a homogeneous model). Field
        ...
        if self.head_dim >= 256 and envs.KVARN_POW2_SLOT:
            ...
            # port(0.27.1): default (relaxed) alignment: multiple of 8 for the
            # fp16 field loads AND of `group` so the per-token slot is integral
            # (the TQFullAttentionSpec page size is block_size * heads * slot,
            # which must equal heads * tile_bytes_aligned exactly).
            unit = math.lcm(8, self.group)
            aligned = ((self.tile_bytes + unit - 1) // unit) * unit
        # port(0.27.1): invariant relied upon by the KV-cache spec / page-size
        # accounting (slot = tile_bytes_aligned // group must be exact).
        assert aligned % self.group == 0, (
            f"KVarN tile_bytes_aligned={aligned} is not a multiple of "
    ...
    # ── fp16 FA materialize-scratch sizing ──────────────────────────────────
    # port(0.27.1): moved here from the backend's `_ensure_pool` so the cap is a
    # single tunable. The materialize (flash_attn_varlen) fallback packs the
    # batch's whole context into a shared fp16 K/V scratch of `fa_scratch_rows`
    # tokens (x Hk x D x 2 tensors x 2 B). The theoretical bound
    # max_num_seqs*max_model_len is pathological, so it is capped at
    # KVARN_FA_SCRATCH_CAP tokens (default 262144 = ~1 GiB at Hk=4, D=256).
    # NOTE: the KVarN original used `max(min(S*L, CAP), L, 4096)`, i.e. the bare
    # `max_model_len` term defeated the cap for a long-context deployment; the
    # helper caps that term too. Contexts longer than the cap fall back to
    # `_decode_path_slow` on the materialize route until segmented materialize
    # lands (plan Phase 2), so lower it only if KV capacity is the constraint.
    FA_SCRATCH_CAP_DEFAULT = 262144
    ...
    @classmethod
    def fa_scratch_rows(cls, max_num_seqs: int, max_model_len: int) -> int:
        """Rows (KV tokens) of the shared fp16 FA materialize scratch."""
        cap = cls.fa_scratch_cap()
        return max(min(max_num_seqs * max_model_len, cap),
